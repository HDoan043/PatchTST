...
class Exp_Main(Exp_Basic):
    ...
    def train(self, setting):
        ...
        print("~~~~~ Numbers of trainable parameters: {} ~~~~~".format(sum(p.numel() for p in self.model.parameters() if p.requires_grad)))
    ...
    def predict(self, setting, load=False):
        ...
        self.model.eval()
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark, batch) in enumerate(pred_loader):
                if self.args.hybrid:
                    batch = batch.float().to(self.device)
                    loss = self.model(batch)
                ...
                if self.args.use_amp:
                    ...
                    if 'Linear' in self.args.model or 'TST' in self.args.model:
                        ...
                        if self.args.output_attention:
                            ...
                            outputs = self.model(batch_x, batch_x_mark, dec_inp, batch_y_mark)
                pred = np.squeeze(outputs.detach().cpu().numpy(), axis = 0)  # .squeeze()
