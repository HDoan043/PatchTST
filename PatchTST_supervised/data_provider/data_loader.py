...
class Time_Series_Practice_Dataset_00(Dataset):
    ...
    def __getitem__(self, index):
        ...
        seq_x = torch.tensor(self.data_x[s_begin:s_end].to_numpy(), dtype=np.float32)
        seq_y = torch.tensor(self.data_y[r_begin:r_end].to_numpy(), dtype=np.float32)
        seq_x_mark = torch.tensor(self.data_stamp[s_begin:s_end].to_numpy(), dtype=torch.float32)
        seq_y_mark = torch.tensor(self.data_stamp[r_begin:r_end].to_numpy(), dtype=torch.float32)
