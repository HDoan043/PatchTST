...
class Dataset_Custom(Dataset):
    ...
    def __getitem__(self, index):
        ...
        seq_x = self.data_x[s_begin:s_end]
        seq_y = self.data_y[r_begin:r_end]
        seq = self.data_x[s_begin:s_begin + self.seq_len + self.pred_len]
        seq_x_mark = self.data_stamp[s_begin:s_end]
        seq_y_mark = self.data_stamp[r_begin:r_end]

        return seq_x, seq_y, seq_x_mark, seq_y_mark, seq
...
class Dataset_Anomaly_Detect(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.set_type == 2:
            ...
            label = np.any(label > 0)                                 # if there is at least a anomal time stamp in range, the range is anomal
            return seq, label
        else: 
            return seq
